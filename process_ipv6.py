...
def process_ipv6_file(input_file):
    private_ips = []
    public_ips = []
    ...
    with open(input_file, 'r') as file:
        for line in file:
            ip = line.strip()
            if is_private_ipv6(ip) is True:
                private_ips.append(ip)
            elif is_private_ipv6(ip) is False:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python process_ipv6.py <input_file>")
        sys.exit(1)

    process_ipv6_file(sys.argv[1])
